...
// métodos da home
export class CrudScreen extends React.Component {
    ...
    atualizaPerfume = (item0, item1, item2, item3, item4, item5) => {
        ...
        PerfumeServico.updateByObjeto(perfume).then((response) => {
    ...
    localizaPerfume = (id) => {
        PerfumeServico.findById(id)
            .then((response) => {
                if (response._array.length > 0 && response != null && response != undefined) {
                    let perfumepesquisa = new Perfume()// cria objeto memória
        ...
        return (
            ...
            <View style={styles.container}>
                ...
                <View style={styles.containerTouch}>
                    <TouchableOpacity onPress={() => { formularioNome == null ? Alert.alert("O campo de nome do perfume não pode ser vazio") : this.insertPerfume(formularioNome, formularioAroma, formularioCategoria, formularioMarca, formularioDescricao) }} style={{ alignItems: "center", backgroundColor: '#f9c2ff' }}>
                        <Icon name="md-add" size={30} color="white" />
                    </TouchableOpacity>
                </View>

                <View style={styles.containerTouch}>
                    <TouchableOpacity onPress={() => { formularioId == null ? Alert.alert("Não tem Objeto para atualizar faça uma pesquisa") : this.atualizaPerfume(formularioId, formularioNome, formularioAroma, formularioCategoria, formularioMarca, formularioDescricao) }} style={{ alignItems: "center", backgroundColor: '#f9c2ff' }}>
                        <Icon name="md-refresh" size={30} color="white" />
                    </TouchableOpacity>
                </View>

                <View style={styles.containerTouch}>
                    <TouchableOpacity onPress={() => { Id_pesquisar == null ? Alert.alert("O campo de id não pode ser vazio") : this.localizaPerfume(Id_pesquisar) }} style={{ alignItems: "center", backgroundColor: '#f9c2ff' }}>
                        <Icon name="md-search" size={30} color="white" />
                    </TouchableOpacity>
                </View>

                <View style={styles.containerTouch}>
                    <TouchableOpacity onPress={() => { formularioId == null ? Alert.alert("O campo de id não pode ser vazio") : this.deletePerfume(Id_pesquisar) }} style={{ alignItems: "center", backgroundColor: '#f9c2ff' }}>
